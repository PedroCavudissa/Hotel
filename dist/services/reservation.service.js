...
export class ReservationService {
    ...
    static async findRoomConflict(roomId, checkIn, checkOut, excludeReservationId) {
        const now = new Date();
    ...
    static async suggestRoom(checkIn, checkOut, capacity, excludeRoomId) {
        ...
        for (const room of rooms) {
            const conflict = await this.findRoomConflict(room.id, checkIn, checkOut);
            if (!conflict)
                return room;
        }
        return null;
    }
    static async assertRoomAvailable(roomId, checkIn, checkOut, excludeReservationId) {
        ...
        const conflict = await this.findRoomConflict(roomId, checkIn, checkOut, excludeReservationId);
        if (conflict) {
            const suggestion = await this.suggestRoom(checkIn, checkOut, room.capacity, roomId);
    ...
    static validateGuestData(data, requireName = false) {
    ...
    static async upsertGuestForUser(userId, data) {
    ...
    static async createWalkInGuest(data, staffId) {
    ...
    static async reschedule(id, data) {
        const reservation = await prisma.reservation.findUnique({
            where: { id },
            include: { room: true },
        });
    ...
    static async changeRoom(id, roomId) {
        if (!roomId)
            throw new AppError("Informe o novo quarto", 400);
    ...
    static async confirmPayment(id, method, amountPaid) {
        const reservation = await prisma.reservation.findUnique({
            where: { id },
            include: reservationInclude,
        });
    ...
    static async uploadPaymentProof(id, paymentProofUrl) {
    ...
    static async checkOut(reservationId, earlyCheckoutReason) {
        const reservation = await prisma.reservation.findUnique({
            where: { id: reservationId },
            include: { room: true },
        });
        ...
        if (now < reservation.checkOut && policy.earlyCheckoutRefundPercent > 0) {
            ...
            const totalHours = (reservation.checkOut.getTime() - reservation.checkIn.getTime()) /
                (1000 * 60 * 60);
            const unusedHours = (reservation.checkOut.getTime() - now.getTime()) / (1000 * 60 * 60);
        ...
        const graceEnd = new Date(reservation.checkOut.getTime() + policy.lateCheckoutGraceMinutes * 60 * 1000);
        if (now > graceEnd) {
            const extraHours = Math.ceil((now.getTime() - graceEnd.getTime()) / (1000 * 60 * 60));
